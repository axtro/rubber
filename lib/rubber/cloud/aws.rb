  ...
  module Cloud
    ...
    class Aws < Base
      ...
      def create_volume(size, zone, base_volume_id = nil)
        snapshot_id = nil
        if base_volume_id
          base_snapshots = @ec2.describe_snapshots["snapshotSet"]["item"].select{|s|s[:aws_volume_id] == base_volume_id}
          snapshot_id = base_snapshots.sort{|a,b| b["startTime"] <=> a["startTime"]}
        end
